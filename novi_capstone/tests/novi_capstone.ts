...
// Configure the client to use the local cluster.
  ...
  const config = findProgramAddressSync([Buffer.from("config")], program.programId)[0];
  ...
  let bonk_mint: PublicKey;
  let wif_mint: PublicKey;

   // ATAs
   let user_usdc: PublicKey; 
   let vault_bonk: PublicKey;
   let vault_wif: PublicKey;  
